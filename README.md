...
1. **更新版本号**：在 `sdk/build.gradle` 中修改 `version` 字段（如：`version = '1.0.5'`）
...
3. **提交代码**：
   ...
   ```
4. **创建Release**：
   - 创建Git标签：`git tag -a v1.0.5 -m "Release version 1.0.5"`
   - 推送标签：`git push origin v1.0.5`
   - 此时JitPack会自动开始构建

## 版本历史

- **1.0.5**: 修复JitPack构建配置，正确发布AAR产物
- **1.0.4**: 修复gradlew换行符问题
- **1.0.3**: 尝试修复gradlew权限和格式问题
- **1.0.2**: 添加Gradle Wrapper支持
- **1.0.1**: 修复JitPack构建配置
- **1.0.0**: 初始版本，包含所有核心功能
